...
--]]
local BotSupplyItemRandomDataTable = {}

local BattleItemDataTable = require("BattleItemDataTable")

BotSupplyItemRandomDataTable.szFileName = "common/ffa/ai/bot/bot_supply_items_random.tab"

function BotSupplyItemRandomDataTable:OnEditorDefine(Parser)
...
function BotSupplyItemRandomDataTable:OnEditorParseLine(Parser, tbContainer, tbNewTemplate)
    ...
    local tbGroupTemplates = tbContainer[tbNewTemplate.nGroupId]
    if not tbGroupTemplates then
        tbGroupTemplates = {}
        tbContainer[tbNewTemplate.nGroupId] = tbGroupTemplates
    end
    ...
    if tbNewTemplate.nItemTemplateId1 > 0 then
        if BattleItemDataTable:GetTemplate(tbNewTemplate.nItemTemplateId1) == nil then
            error("Cannot find item template! groupid:"..tbNewTemplate.nGroupId..", item template id: "..tbNewTemplate.nItemTemplateId1)
        end
        if tbNewTemplate.nItemCount1 <= 0 then
            error("item count less than 0!groupid:"..tbNewTemplate.nGroupId..", item template id: "..tbNewTemplate.nItemTemplateId1..", count:"..tbNewTemplate.nItemCount1)
        end
    end
    if tbNewTemplate.nItemTemplateId2 > 0 then
        if BattleItemDataTable:GetTemplate(tbNewTemplate.nItemTemplateId2) == nil then
            error("Cannot find item template! groupid:"..tbNewTemplate.nGroupId..", item template id: "..tbNewTemplate.nItemTemplateId2)
        end
        if tbNewTemplate.nItemCount2 <= 0 then
            error("item count less than 0!groupid:"..tbNewTemplate.nGroupId..", item template id: "..tbNewTemplate.nItemTemplateId1..", count:"..tbNewTemplate.nItemCount2)
        end
    end
    return true
end
...
-- [EXPORT BEGIN]
function BotSupplyItemRandomDataTable:GetRandomItems(nGroupId)
    local tbGroupTemplates = self.tbContainer[nGroupId]
    if tbGroupTemplates == nil then
        error("BotSupplyItemRandomDataTable:GetRandomItems failed, nGroupId:".. nGroupId)
    end
    local tbRandomItems = {}
...
return BotSupplyItemRandomDataTable
