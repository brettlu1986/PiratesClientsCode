...
--]]
local VehicleCameraDataTable = {}

local StringUtil = require("StringUtil")

VehicleCameraDataTable.szFileName = "common/human/vehicle_camera.tab"

function VehicleCameraDataTable:OnEditorDefine(Parser)
    Parser:SetKey("nId")
...
function VehicleCameraDataTable:OnEditorParseLine(Parser, tbContainer, tbNewTemplate)
...
function VehicleCameraDataTable:GetTemplate(nTemplateId)
    return self.tbContainer[nTemplateId]
end

function VehicleCameraDataTable:GetVehicleInitCameraParam(nVehicleId)
    local tbParam = self:GetTemplate(nVehicleId)
    if tbParam then  
        return {
