...
local ULScheduleFixedAward = luaclass("ULScheduleFixedAward", UILogicBase)
...
ULScheduleFixedAward.tbItems = nil
...
local function RefreshIndex(self, pWidgetRef, tbTemplate, nState, nIndex)
    ...
    pWidgetRef["imgFxGlow"..nIndex]:SetVisibility(ESlateVisibility_Hidden)
...
function ULScheduleFixedAward:Activate(tbAllWidget)
    local pWidgetRef = self.pWidgetRef
    ...
    -- pWidgetRef.txtDesc1:SetText(tbTemp.l10nDesc)
    OnRefresh(self)
end

function ULScheduleFixedAward:Deactivate()
end

function ULScheduleFixedAward:OnLoad()
    local pWidgetRef = self.pWidgetRef
...
function ULScheduleFixedAward:OnDestroy()
    self.tbItems = nil
end

function ULScheduleFixedAward:OnBindEvent(EventHelper)
    for i = 1, MAX_COUNT do
