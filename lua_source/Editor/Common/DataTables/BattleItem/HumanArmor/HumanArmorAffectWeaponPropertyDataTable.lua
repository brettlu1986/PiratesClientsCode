-----------------------------------------------------
--File Name    : HumanArmorAffectWeaponPropertyDataTable.lua
--Author       : WuJizhou
--Create Time  : 3/26/2020, 5:38:01 PM
--Description  : HumanArmorAffectWeaponPropertyDataTable
-----------------------------------------------------
local HumanArmorAffectWeaponPropertyDataTable = {}

local HumanArmorDef = require("HumanArmorDef")
local ArmorAffectHumanWeaponPropertyDef = HumanArmorDef.ArmorAffectHumanWeaponPropertyDef
local ArmorAffectActionDef = HumanArmorDef.ArmorAffectActionDef

HumanArmorAffectWeaponPropertyDataTable.szFileName = "common/ffa/item/human_armor/affect_weapon_property.tab"

local MAX_AFFECT_PROPERTY_COUNT = 5

function HumanArmorAffectWeaponPropertyDataTable:OnEditorDefine(Parser)
    Parser:SetKey("nId")
    Parser:Define("nId", "id", -1, Parser.TypeInt)

end

function HumanArmorAffectWeaponPropertyDataTable:OnEditorParseLine(Parser, tbContainer, tbNewTemplate)
    ...
    for i = 1, MAX_AFFECT_PROPERTY_COUNT do
        ...
        if szProperty ~= "" then
            local szPropName = ArmorAffectHumanWeaponPropertyDef[szProperty]
...
-- [EXPORT BEGIN]
--return list
function HumanArmorAffectWeaponPropertyDataTable:GetAffectProperties(nId)
    local tbTemplate = self.tbContainer[nId]
    return tbTemplate.tbProperty
end
-- [EXPORT END]

return HumanArmorAffectWeaponPropertyDataTable