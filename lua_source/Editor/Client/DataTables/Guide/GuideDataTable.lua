...
local GuideDataTable = {}

GuideDataTable.szFileName = "client/guide/guide.tab"
GuideDataTable.bEnableIterateKey = true

-- [EXPORT]
GuideDataTable.tbModuleContainer = {}

function GuideDataTable:OnEditorDefine(Parser)
...
function GuideDataTable:OnEditorParseLine(Parser, tbContainer, tbNewTemplate)
...
function GuideDataTable:OnEditorParseFinished()
    
end

-- [EXPORT BEGIN]
function GuideDataTable:GetModuleGroups(nModuleId)
...
-- [EXPORT BEGIN]
function GuideDataTable:GetGroupType(nModuleId, nGroupId)
...
-- [EXPORT BEGIN]
function GuideDataTable:GetGroupIsInQueue(nModuleId, nGroupId)
    local tbModule = self.tbModuleContainer[nModuleId]
    if(tbModule == nil)then
        return false
    end
...
-- [EXPORT BEGIN]
function GuideDataTable:GetModules(tbModuleAndGroup)
    local tbModuleContainer = self.tbModuleContainer
...
-- [EXPORT BEGIN]
function GuideDataTable:GetAllModules()
    local tbModuleContainer = self.tbModuleContainer
    if not tbModuleContainer then
        return nil
    end
    -- log("===============GetAllModules===============")
    -- for k,tbModule in pairs(tbModuleContainer) do
    --     for j,tbGroup in pairs(tbModule) do
    --         for index, tbStep in pairs(tbGroup) do
    --             print(k, j, index, tbStep.bIsModal, tbStep.bDefensive, tbStep.bAlwaysTrigger)
    --         end
    --     end
    -- end
...
-- [EXPORT BEGIN]
function GuideDataTable:GetAllSteps()
