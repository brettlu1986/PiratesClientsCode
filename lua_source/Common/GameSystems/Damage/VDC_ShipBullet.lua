-----------------------------------------------------
--File Name    : VDC_ShipBullet.lua
--Author       : chenyiixn
--Create Time  :
--Description  : 计算船对马的伤害
-----------------------------------------------------
...
return function (tbTaker, nActualDamage, pDamageCauser, pHitResult)
    ...
    if not WeaponItem then
        logerror("[VDC_ShipBullet] Cannot find WeaponItem ", nWeaponId)
        return
    end
    local tbWeaponTemplate = WeaponItem:GetTemplate()
    if not tbWeaponTemplate then
        logerror("[VDC_ShipBullet] Cannot find tbWeaponTemplate ", nWeaponId)
        return
    end
    ...
    PropUtil.ApplyDamage(tbTaker, tbCauser, nDamageType, nActualDamage, tbDamageExtraData)
end