...
local JGMCaptureFlagSetting = luaclass("JGMCaptureFlagSetting", JGMCommonSetting)
...
JGMCaptureFlagSetting.FirstGroupIndex = nil           -- 第一组id
JGMCaptureFlagSetting.AnotherPlayerStartAction = nil  -- 另一组StartAction 
JGMCaptureFlagSetting.tbPlayerWaitRevive = nil        -- 玩家死亡等待复活
JGMCaptureFlagSetting.ReviveAction = nil              -- 玩家复活执行Action
-- JGMCaptureFlagSetting.RemainTimeTimer = nil           -- BattleSetStepRemainTimeAction中同步处理
-- local UPDATE_INTERVAL = 10                             -- 同步间隔
-- local WAIT_FOR_PLAY = 1                            -- 等待游戏开始时间

function JGMCaptureFlagSetting:Init(tbGameMode)
    assert(not GlobalVariableSystem:IsClient(), "Enter Dungeon mode error")

    if(not JGMCaptureFlagSetting.super.Init(self, tbGameMode)) then
        return false
    end
...
function JGMCaptureFlagSetting:Parse(tbJsonData)
    local tbStep
    ...
    JGMCaptureFlagSetting.super.Parse(self, tbJsonData)
...
function JGMCaptureFlagSetting:OnFindPlayerStart(tbPlayer)
    ...
    return JGMCaptureFlagSetting.super.OnFindPlayerStart(self, tbPlayer)
end
...
function JGMCaptureFlagSetting:Uninit()
    ...
    JGMCaptureFlagSetting.super.Uninit(self)
end

function JGMCaptureFlagSetting:GetQuitDungeonDialogType()
    return DungeonQuitDialogType.CaptureFlag
end

function JGMCaptureFlagSetting:OnPawnDead(tbDeadActor)
...
function JGMCaptureFlagSetting:OnStartStep(tbStep)
    if self.ResultStep == tbStep then
        self.tbGameMode:OnPawnsPaused()
        ClearReviveData(self)
    end
    JGMCaptureFlagSetting.super.OnStartStep(self, tbStep)
end

function JGMCaptureFlagSetting:OnBatterReviveSuccess(tbPlayer)
...
function JGMCaptureFlagSetting:OnSnapshotGameState()
    JGMCaptureFlagSetting.super.OnSnapshotGameState(self)
    
    local rBattleFlagState = self.tbGameMode.tbGameState.rBattleFlagState
    if rBattleFlagState.tbStateInfos then
        rBattleFlagState.Rep()
    end
end

-- function JGMCaptureFlagSetting:StepRemainTimeSync(nRemainTime)
