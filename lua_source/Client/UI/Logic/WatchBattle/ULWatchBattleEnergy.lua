...
local ULWatchBattleEnergy = luaclass("ULWatchBattleEnergy", UILogicBase)
...
ULWatchBattleEnergy.nCurrentHpLevel = nil
ULWatchBattleEnergy.tbEpSplitWidget = nil
...
local function OnWatchMateHpChanged(self, _, _, nHpPercent)
    ...
    -- logdebug("the hp percent ::", nHpPercent, nCurrentHpLevel, self.nCurrentHpLevel)
...
local function OnWatchMateDyingChanged(self)
    local Owner = self.Owner
    if Owner.tbCurrrentWatchObj and not Owner.tbCurrrentWatchObj:IsDead() then
...
function ULWatchBattleEnergy:RefreshCurrentMateEnergy()
    ...
    if Owner.tbLastWatchObj then
        ...
        if LastPropertyComponent then
            ...
            EventHelper:UnregisterLuaDelegate(LastPropertyComponent.OnIsDyingChanged, OnWatchMateDyingChanged, self)
        end
    end
    ...
    if Owner.tbCurrrentWatchObj and not Owner.tbCurrrentWatchObj:IsDead() then
        ...
        if PropertyComponent then
            ...
            EventHelper:RegisterLuaDelegate(PropertyComponent.OnIsDyingChanged, OnWatchMateDyingChanged, self)
        end
    end
end

function ULWatchBattleEnergy:OnLoad()
...
function ULWatchBattleEnergy:OnEnter()
    --Owner is UIWatchBattle
    --local Owner = self.Owner
end

function ULWatchBattleEnergy:OnBindEvent(EventHelper)
end

function ULWatchBattleEnergy:OnUnload()
    --unload res
end

return ULWatchBattleEnergy