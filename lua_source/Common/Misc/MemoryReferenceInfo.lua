...
local function CollectObjectReferenceInMemory(strName, cObject, cDumpInfoContainer)
	...
	if "table" == strType then
		...
		for k, v in pairs(cObject) do
			...
			elseif "boolean" == strKeyType then
				CollectObjectReferenceInMemory(strName .. ".[table:value]", v, cDumpInfoContainer)
			else
...
local function WriteFileLines(tbData, strFileName)
	return ExtendBlueprintFunctions.WriteFileLines(tbData, strFileName)
end
...
local function OutputMemorySnapshot(strSavePath, strExtraFileName, nMaxRescords, strRootObjectName, cRootObject, cDumpInfoResultsBase, cDumpInfoResults)
	...
	local bWriteResult = true
	local nLineCount = 2000
	local tbData = {}
	local cOutputer = function (strContent)
		table.insert(tbData, strContent)
		if (#tbData >= nLineCount) then
			if bWriteResult then
				bWriteResult = WriteFileLines(tbData, strFileName)
				if not bWriteResult then 
					logerror("write mem file failed ", strFileName)
				end
			end
			tbData = {}
		end
	...
	if #tbData > 0 and #tbData < nLineCount then
		if bWriteResult then
			WriteFileLines(tbData, strFileName)
		end
	end 
	-- if bOutputFile then
