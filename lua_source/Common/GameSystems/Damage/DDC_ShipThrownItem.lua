-----------------------------------------------------
--File Name    : SDD_ShipThrownItem.lua
--Author       : fang jing
...
return function(tbTaker, nActualDamage, pDamageCauser, pHitResult)
    ...
    if not tbCauser then
        logerror("[DDC_ShipThrownItem] Cannot find Causer")
        return
    end
    ...
    if not tbThrownItemtemplate then
        logerror("[DDC_ShipThrownItem] Cannot find ThrownItemtemplate, nThrownItemTemplateId =", nThrownItemTemplateId)
        return
    end
    ...
    tbTaker.DestructibleObjectPropertyComponent:ApplyDamage(tbCauser, nDamageType, nFinalDamage, tbDamageExtraData)
end